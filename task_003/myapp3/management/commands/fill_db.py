...
class Command(BaseCommand):
    help = "Generate fake customer and posts."
    ...
    def handle(self, *args, **kwargs):
        ...
        for i in range(1, count + 1):
            customer = Customer(name=f'Customer_{i}', email=f'mail{i}@mail.ru')
            customer.save()
            for j in range(1, count + 1):
                product = Product(name=f'Title-{j}', price=" ".join(choices(text, k=64)), customer=customer)
                product.save()